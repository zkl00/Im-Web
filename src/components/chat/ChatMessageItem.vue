...
export default {
	...
	computed: {
		...
		contentData() {
			try {
				if (typeof this.msgInfo.content === 'string') {
					return JSON.parse(this.msgInfo.content);
				}
				return this.msgInfo.content || {};
			} catch (e) {
				console.error('[消息解析失败] content:', this.msgInfo.content, 'error:', e);
				return {};
			}
		},
		fileSize() {
		...
		menuItems() {
			...
			if (this.msgInfo.selfSend && this.msgInfo.id) {
				items.push({
					key: 'RECALL',
					name: '撤回',
