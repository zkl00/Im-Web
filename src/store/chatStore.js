...
export default defineStore('chatStore', {
	...
	actions: {
		...
		insertMessage(msgInfo, chatInfo) {
			...
			if (message) {
				// 保留本地已有的 seq 值（发送响应返回的 seq 优先级更高）
				if (message.seq && !msgInfo.seq) {
					msgInfo.seq = message.seq;
				}
		...
		loadConversationList() {
			return new Promise((resolve, reject) => {
				...
				this.loadChat().then(() => {
					...
					const conversationElems = data?.conversationElems || [];

					conversationElems.forEach(elem => {
						const msgInfo = elem.msgInfo || {};
						const sessionType = msgInfo.sessionType;

						// sessionType: 1=单聊, 2=群聊, 3=大群, 4=通知会话
						let type, targetId, showName, headImage;
						switch (sessionType) {
							case 1: // 单聊
								type = 'PRIVATE';
								targetId = msgInfo.sendID === userId ? msgInfo.recvID : msgInfo.sendID;
								showName = msgInfo.senderName;
								headImage = msgInfo.faceURL;
								break;
							case 2: // 群聊
							case 3: // 大群
								type = 'GROUP';
								targetId = msgInfo.groupID;
								showName = msgInfo.groupName;
								headImage = msgInfo.groupFaceURL;
								break;
							case 4: // 通知会话
								type = 'NOTIFICATION';
								targetId = elem.conversationID;
								showName = '系统通知';
								headImage = '';
								break;
							default:
						...
						// 查找本地缓存中是否已有该会话
						let existingChat = this.chats.find(c => c.type === type && c.targetId === targetId);

						if (existingChat) {
							// 更新已存在的会话信息，保留本地消息
							existingChat.showName = showName || existingChat.showName;
							existingChat.headImage = headImage || existingChat.headImage;
							existingChat.isDnd = elem.recvMsgOpt !== 0;
							existingChat.lastContent = this.parseContent(msgInfo.contentType, msgInfo.content) || existingChat.lastContent;
							existingChat.lastSendTime = msgInfo.LatestMsgRecvTime || existingChat.lastSendTime;
							existingChat.unreadCount = elem.unreadCount || 0;
							existingChat.isPinned = elem.IsPinned || false;
							existingChat.conversationID = elem.conversationID;
							existingChat.sessionType = sessionType;
							existingChat.stored = false;
						} else {
							// 新会话，添加到列表
							const chat = {
								targetId: targetId,
								...
								showName: showName || '',
								headImage: headImage || '',
								isDnd: elem.recvMsgOpt !== 0,
								lastContent: this.parseContent(msgInfo.contentType, msgInfo.content),
								lastSendTime: msgInfo.LatestMsgRecvTime || Date.now(),
								unreadCount: elem.unreadCount || 0,
								hotMinIdx: 0,
								readedMessageIdx: 0,
								messages: [],
								atMe: false,
								...
								stored: false,
								delete: false,
								isPinned: elem.IsPinned || false,
								conversationID: elem.conversationID
							};
							this.chats.push(chat);
						}

						// 如果是群聊，添加到 groupStore
						if ((sessionType === 2 || sessionType === 3) && msgInfo.groupID) {
							groupStore.addGroup({
								id: msgInfo.groupID,
		...
		parseContent(contentType, content) {
			...
			try {
				...
				switch (contentType) {
					case 101: return parsed.text || parsed.content || '';
					case 102: return '[图片]';
					case 103: return '[语音]';
					case 104: return '[视频]';
					case 105: return '[文件]';
	...
	getters: {
		...
		findMessage: (state) => (chat, msgInfo) => {
			...
			for (let idx = chat.messages.length - 1; idx >= 0; idx--) {
				let m = chat.messages[idx];
				// 通过 id 匹配
				if (msgInfo.id && m.id && msgInfo.id == m.id) {
					return m;
				}
				// 通过 clientMsgID 匹配（用于自己发送的消息）
				if (msgInfo.clientMsgID && m.clientMsgID && msgInfo.clientMsgID == m.clientMsgID) {
					return m;
				}
				// 通过 tmpId 匹配（发送中的消息）
				if (msgInfo.tmpId && m.tmpId && msgInfo.tmpId == m.tmpId) {
					return m;
				}
				// 限制搜索范围，只查找最近100条消息
				if (chat.messages.length - idx > 100) {
					break;
				}
			}
			return null;
